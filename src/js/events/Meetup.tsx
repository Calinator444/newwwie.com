import { Component, ComponentChildren, render as preactRender } from "preact";
import { Group, Image, Venue } from "./types";
import { events } from "./events-data";

/**
 * TS ignore is required below only as the getElementById could return
 * nothing. In this instance as long as we ensure that index.html
 * includes a html node with this id, this is a safe ignore.
 */
// @ts-ignore
...
export class Meetup extends Component {
  ...
  async render() {
    ...
    if (events.length !== 0) {
      const eventsNode = events.map(({ event, group }) => {
      ...
      preactRender(eventsNode, eventIdNode);
    }
    if (events.length === 0) {
      preactRender(
        <li>
  ...
  parseIsoDuration(isoDuration: string) {
    ...
    const matches = isoDuration.match(DURATION_REGEX);
